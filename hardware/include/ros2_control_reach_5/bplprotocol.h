...
size_t decodeFloats(float *outputFloatBuffer, uint8_t *inputBuffer, size_t inputBufferLength)
{
    ...
    uint32_t asInt;
    for (int i = 0; i < static_cast<int>(floatLength); i++)
    {
        asInt = 0;
...
/**
 * @brief Decodes multiple packets from an array of bytes.
 *
 * @param packets Array of Packet structures to fill data into.
 * @param inputData Pointer to the list of bytes.
 * @param inputDatalength Length of the bytes.
 * @return int Number of packets decoded, -1 if error.
 */
int decodePackets(struct Packet *packets, uint8_t *inputData, size_t inputDatalength)
{
    if (inputDatalength > MAX_PACKET_LENGTH)
    {
        return -1; // Too much data for buffer
    }

    int packetCount = 0;
    size_t currentIndex = 0;

    while (currentIndex < inputDatalength)
    {
        size_t packetEndIndex = currentIndex;
        while (packetEndIndex < inputDatalength && inputData[packetEndIndex] != 0x00)
        {
            packetEndIndex++;
        }

        if (packetEndIndex - currentIndex > MAX_PACKET_LENGTH)
        {
            return -1; // Single packet too large
        }

        if (packetEndIndex != currentIndex)
        {
            uint8_t tempBuffer[MAX_PACKET_LENGTH];
            memcpy(tempBuffer, &inputData[currentIndex], packetEndIndex - currentIndex);

            int decoded = decodePacket(&packets[packetCount], tempBuffer, packetEndIndex - currentIndex);
            if ((decoded < 0) && (packetCount == 0))
            {
                return -1; // Error decoding packet
            } else if (decoded > 0)
            {
                packetCount++;
            }
        }
        currentIndex = packetEndIndex + 1; // Skip the 0x00 byte
    }
